  ...
  try {
    ...
    if (!['individual_notice', 'group_notice'].includes(type)) {
      return res.status(400).json({ 
        _status: 'FAIL', 
        _error: { code: 'INVALID_TYPE', meta: { type } },
        data: null 
      })
    }
...
router.post('/announcements', async (req, res) => {
  try {
    ...
    if (!publisher) {
      return res.status(400).json({ 
        _status: 'FAIL', 
        _error: { code: 'PUBLISHER_NOT_FOUND' },
        data: null 
      })
    }
  ...
  try {
    ...
    if (!announcement) {
      return res.status(404).json({ 
        _status: 'FAIL', 
        _error: { code: 'ANNOUNCEMENT_NOT_FOUND' },
        data: null 
      })
    }

  ...
  try {
    ...
    const todayStats = await sequelize.query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM individual_reservations WHERE DATE(visit_date) = :today AND status = 'pending') as individual_pending_approval,
        (SELECT COUNT(*) FROM group_reservations WHERE DATE(visit_date) = :today AND status = 'pending') as group_pending_approval,
        (SELECT COUNT(DISTINCT license_plate) FROM individual_reservations WHERE DATE(visit_date) = :today AND status = 'approved' AND license_plate IS NOT NULL) as individual_vehicles,
        (SELECT COUNT(DISTINCT license_plate) FROM group_reservations WHERE DATE(visit_date) = :today AND status = 'approved' AND license_plate IS NOT NULL) as group_vehicles
    `, {
    ...
    const todayApproved = parseInt(stats.individual_approved) + parseInt(stats.group_approved)
    const todayPeople = parseInt(stats.individual_approved) + parseInt(stats.group_people)
    const todayPending = parseInt(stats.individual_pending) + parseInt(stats.group_pending)
    const todayCompleted = parseInt(stats.individual_completed) + parseInt(stats.group_completed)
    const todayPendingApproval = parseInt(stats.individual_pending_approval) + parseInt(stats.group_pending_approval)
    const todayVehicles = parseInt(stats.individual_vehicles) + parseInt(stats.group_vehicles)
